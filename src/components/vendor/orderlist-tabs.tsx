...
import { VendorOrderlistRowLoading } from "src/components/vendor/vendor-orderlist-row-loading";
...
export const PurchaseOrderTableCard: FC<CaustomerTableCardProps> = (props) => {
  ...
  const { find, updateEntity, loading, entities } = useEntity("purchase-order");
  ...
  const transformedData = useMemo(() => {
    ...
    entities?.results?.map((d) => {
      ...
      arr.push({
        ...
        subtotal: (
          <Typography variant="body2">{d?.billing.total || 0}</Typography>
        ),
        vendorPayment: (
          <Typography
            variant="body2"
  ...
  return (
    <>
      <Card>
        ...
        <PoGrnTable
          isLoading={loading}
          loaderComponent={VendorOrderlistRowLoading}
