...
export const VariantsReportTopCard: FC<ProductTopCardProps> = (props) => {
  const { t } = useTranslation();
  const { variantStats, loading } = props;
    ...
    <Grid container spacing={3} sx={{ mt: 3 }}>
      <Grid item lg={4} md={6} sm={12} sx={{ width: "100%" }}>
        <CardWithIconDescription
          ...
          heading={variantStats?.totalProducts?.totalProducts || 0}
          description={`${t("Total Variants")}: ${
            variantStats?.totalProducts?.totalVariants || 0
          }`}
          showButton={false}
        />
      </Grid>
      ...
      <Grid item lg={4} md={6} sm={12} sx={{ width: "100%" }}>
        <CardWithIconDescription
          ...
          heading={variantStats?.topPerformingProduct?.productNameEn || "N/A"}
          description={`${t("Total Gross Sale")}: ${currency} ${currencyValue(
            variantStats?.topPerformingProduct?.totalSales?.toFixed(2) || 0.0
          )}`}
          showButton={false}
        />
      </Grid>
      ...
      <Grid item lg={4} md={6} sm={12} sx={{ width: "100%" }}>
        <CardWithIconDescription
          ...
          heading={variantStats?.todaysRefund?.todaysRefundedItems || 0}
          description={`${t("Refund Value")}: ${currency} ${currencyValue(
            variantStats?.todaysRefund?.totalRefundedAmount || 0
          )}`}
          showButton={false}
        />
      </Grid>
