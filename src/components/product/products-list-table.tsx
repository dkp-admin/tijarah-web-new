...
export const ProductListTable: FC<ProductListTableProps> = (props) => {
  ...
  const getKitchenInfo = (product: any) => {
    if (
      !(
        user?.company?.configuration?.enableKitchenManagement ||
    ...
    if (product?.kitchenRefs && product.kitchenRefs.length > 0) {
      return product.kitchens[0]?.name;
    }
  ...
  console.log(user, companyContext);
  return (
    <div>
      <Scrollbar>
                ...
                <Fragment key={product._id}>
                  <TableRow hover key={product._id}>
                    ...
                    {(user?.company?.configuration?.enableKitchenManagement ||
                      ...
                      <TableCell>{getKitchenInfo(product)}</TableCell>
                    )}
