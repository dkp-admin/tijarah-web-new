...
import { VATRatesRowLoading } from "src/components/vat-rates/vat-rates-row-loading";
...
const Page: PageType = () => {
  ...
  const { find, loading, entities, updateEntity } = useEntity("currency");
  ...
  const transformedData = useMemo(() => {
    ...
    entities?.results?.map((d) => {
      arr.push({
        ...
        symbol: <Typography variant="body2">{`${d.symbol}`}</Typography>,
        status: (
          <FormControlLabel
            sx={{
              width: "120px",
        ...
        action: (
          ...
          >
            <IconButton
              onClick={() => {
                router.push({
                  pathname: tijarahPaths.platform.currencies.create,
                  query: {
                    id: d._id,
                  },
                });
        ...
        <Container maxWidth="xl">
          <Stack spacing={3}>
            <Stack direction="row" justifyContent="space-between" spacing={4}>
              <Stack spacing={1}>
                <Typography variant="h4">{t("Currencies")}</Typography>
              </Stack>
            ...
            <Card>
              ...
              <SuperTable
                isLoading={loading}
                loaderComponent={VATRatesRowLoading}
