import { Clear, RestaurantMenu } from "@mui/icons-material";
import {
  Box,
  Button,
  Container,
...
const Menu = () => {
  ...
  console.log(
    "entity?.qrOrderingConfiguration?.qrOrdering",
    entity?.qrOrderingConfiguration?.qrOrdering
  );

  useEffect(() => {
    if (searchParams.get("locationRef")) {
      find({
  ...
  const groupByCategory = () => {
    ...
    return entities?.results?.reduce((result, item) => {
      const categoryRef = item.categoryRef;
      const category = entities?.categories?.find(
        (category: any) => category._id === categoryRef
      );
      ...
      if (!result[categoryRef]) {
        result[categoryRef] = {
          category: {
            name: {
              en: category?.name?.en || item.category.name,
              ar: category?.name?.ar || item.category.name,
            },
          },
          products: [],
        };
      ...
      result[categoryRef].products.push(item);
  ...
  useEffect(() => {
    window.localStorage.setItem("industry", "retail");
    window.localStorage.setItem("qrOrdering", "true");
  ...
  return (
    <>
      <Seo title={t("QR Ordering Retail")} />
            ...
            {entity?.qrOrderingConfiguration?.qrOrdering &&
              ...
              totalAmount > 0 && (
                ...
                >
                  <Fab
                    onClick={() => {
                      if (!entity?.pickupQRConfiguration?.pickup) {
                        return toast.error(
            ...
            {entity && (
              ...
              >
                ...
                >
                  ...
                  >
                    <SvgIcon style={{ width: 22, height: 22, marginRight: 8 }}>
                      <SearchMdIcon
                ...
                {entity?.qrOrderingConfiguration?.qrOrdering && showSearch && (
                  <Stack
                    alignItems="center"
                    component="form"
                    direction="row"
                  ...
                  >
                    ...
                    >
                      ...
                      <Input
                        ...
                        fullWidth
                        value={queryText}
                        placeholder={t("Search by product name, code or SKU")}
                        sx={{ flexGrow: 1 }}
                        onChange={(e) => {
                          handleQueryChange(e);
                        }}
